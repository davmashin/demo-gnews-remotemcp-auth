...
# Authentication middleware (detailed comments made in the 6 folder, same auth file)
class AuthMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            # if validation options error
            except Exception:
                raise HTTPException(status_code=401, detail="Token validation failed")

        # this is the most important one: client willr eceive not only the 401 but also the header with the information about WHERE to get it (token), looking at the metadata endpoint! Then we return the enxt call and we're done.
