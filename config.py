...
class Settings:
    ...
    SCALEKIT_AUTHORIZATION_SERVERS: str = os.environ.get(
    ...
    def __post_init__(self):
        ...
        if not self.SCALEKIT_RESOURCE_METADATA_URL:
            raise ValueError(
                "SCALEKIT_RESOURCE_METADATA_URL environment variable not set"
            )
        if not self.SCALEKIT_AUTHORIZATION_SERVERS:
            raise ValueError(
                "SCALEKIT_AUTHORIZATION_SERVERS environment variable not set"
            )
        if not self.SCALEKIT_RESOURCE_NAME:
            raise ValueError("SCALEKIT_RESOURCE_NAME environment variable not set")
